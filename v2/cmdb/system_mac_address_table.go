...
func (c *Client) CreateSystemMacAddressTable(payload *models.SystemMacAddressTable, params *models.CmdbRequestParams) (*models.CmdbResponse, error) {
	...
	mkey := ""
	if payload.Mac != nil && *params.AllowAppend {
		mkey = *payload.Mac
		read, err := c.ReadSystemMacAddressTable(mkey, params)
		if err != nil {
			return nil, err
		}
		if read != nil {
			log.Printf("[WARN] Resource at path %q with mkey %q detected upon CREATE with flag set to to overwrite. Changing to UPDATE.", models.SystemMacAddressTablePath, mkey)
			return c.UpdateSystemMacAddressTable(mkey, payload, params)
		}
	}
...
func (c *Client) ReadSystemMacAddressTable(mkey string, params *models.CmdbRequestParams) (*models.SystemMacAddressTable, error) {
	...
	req.Mkey = &mkey
	req.Payload = nil
	req.Path = models.CmdbBasePath + models.SystemMacAddressTablePath + url.QueryEscape(mkey) + "/"
	req.Params = *params
